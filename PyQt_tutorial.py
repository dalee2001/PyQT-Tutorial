...
class PyQtApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("OOP PyQt App")     # App name
    ...
    def submit_action(self):
        ...
        if user_input:
            QMessageBox.information(self, "Input Recieved", f"Hello, {user_input}")       # If the user put input, put a message in the window widget saying hello, their name
        else:
            QMessageBox.warning(self, "No Input", "Please enter your name.")      # Warning if user puts no input
